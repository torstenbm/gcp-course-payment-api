...
Point `gcloud` at your project and authenticate:
...
Cloud Build uses the default Compute Engine service account, which needs permissions for Cloud Storage (staging source), Artifact Registry (pushing images), and Cloud Logging:
...
```bash
gcloud artifacts repositories create payment-core \
...
Cloud Build uploads your source, builds the Dockerfile remotely, and pushes the image to Artifact Registry:

```bash
gcloud builds submit \
  --tag europe-north1-docker.pkg.dev/YOUR_PROJECT/payment-core/payment-core-api
```
...
gcloud run deploy payment-core-api \
  --image europe-north1-docker.pkg.dev/YOUR_PROJECT/payment-core/payment-core-api \
  --region europe-north1 \
  --add-cloudsql-instances YOUR_PROJECT:europe-north1:YOUR_INSTANCE \
  --set-env-vars "\
SPRING_CLOUD_GCP_PROJECT_ID=YOUR_PROJECT,\
SPRING_CLOUD_GCP_SQL_INSTANCE_CONNECTION_NAME=YOUR_PROJECT:europe-north1:YOUR_INSTANCE,\
DB_NAME=payments,\
DB_USER=postgres,\
DB_PASSWORD=secret"
```
